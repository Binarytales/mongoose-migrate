...
Set.prototype.save = function(fn){
  ...
  var env = process.env.NODE_ENV || 'development',
      configFile = process.env.NODE_MIGRATIONS_CONFIG || './config.js';

  var config = require(configFile)[env];

  config = {
    db: config.db.url,
    model: config.migrations.model || 'Migrations'
  }

  mongoose.connect(config.db, function () {
    var Migration = mongoose.model(config.model)
...
Set.prototype.load = function(fn){
  ...
  var env = process.env.NODE_ENV || 'development',
      configFile = process.env.NODE_MIGRATIONS_CONFIG || __dirname + '/../config.js';
  ...
  config = {
    db: config.db.url,
    model: config.migrations.model || 'Migrations',
    schema: config.migrations.schema || { "migration": {} }
  }

  mongoose.connect(config.db, function () {
    var Schema = mongoose.Schema;
    var MigrationSchema = new Schema(config.schema);
    var Migration = mongoose.model(config.model, MigrationSchema)
